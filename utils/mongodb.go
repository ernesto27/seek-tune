...
func (db *MongoDB) StoreFingerprints(fingerprints map[uint32]models.Couple) error {
...
func (db *MongoDB) GetCouples(addresses []uint32) (map[uint32][]models.Couple, error) {
	collection := db.client.Database("song-recognition").Collection("fingerprints")
...
func (db *MongoDB) TotalSongs() (int, error) {
...
func (db *MongoDB) RegisterSong(songTitle, songArtist, ytID string) (uint32, error) {
	existingSongsCollection := db.client.Database("song-recognition").Collection("songs")
...
func (db *MongoDB) GetSong(filterKey string, value interface{}) (s Song, songExists bool, e error) {
...
func (db *MongoDB) GetSongByID(songID uint32) (Song, bool, error) {
	return db.GetSong("_id", songID)
}

func (db *MongoDB) GetSongByYTID(ytID string) (Song, bool, error) {
	return db.GetSong("ytID", ytID)
}

func (db *MongoDB) GetSongByKey(key string) (Song, bool, error) {
	return db.GetSong("key", key)
}

func (db *MongoDB) DeleteSongByID(songID uint32) error {
	songsCollection := db.client.Database("song-recognition").Collection("songs")
...
func (db *MongoDB) DeleteStorage(collectionName string) error {
...
func (db *MongoDB) GetStorageName() string {
	return "MongoDB"
}
